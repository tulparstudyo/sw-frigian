...
<table> 
	...
	<thead>
		<tr>
			...
			<?php if( $modify ) : ?>
				<th class="action"><?= $enc->html( $this->translate( 'client', 'ACTION' ), $enc::TRUST ); ?></th>
			<?php endif; ?>
		</tr>
	</thead>
			...
			<tr class="product <?= ( isset( $errors['product'][$position] ) ? 'error' : '' ); ?>">
				...
				<td class="details">
					...
					<?php if( ( $desc = $product->getDescription() ) !== '' ) : ?>
						<p class="product-description"><?= $enc->html( $desc , $enc::TRUST ); ?></p>
					<?php endif ?>
				...
				<?php if( $modify ) : ?>
					<td class="action">
						<?php if( ( $product->getFlags() & \Aimeos\MShop\Order\Item\Base\Product\Base::FLAG_IMMUTABLE ) == 0 ) : ?>
							...
							<a class="minibutton delete" href="<?= $enc->attr( $this->url( $basketTarget, $basketController, $basketAction, $basketParams, [], $basketConfig ) ); ?>"><i class="fa fa-times"></i></a>
						<?php endif; ?>
					</td>
				<?php endif; ?>
	...
	<tfoot>
		...
		<?php if( ( $costs = $this->get( 'summaryCostsPayment', 0 ) ) > 0 ) : ?>
			<tr class="payment">
			<td colspan="4"></td>
				<td ><?= $enc->html( $this->translate( 'client', 'Payment costs' ) ); ?></td>
				<td class="price"><?= $enc->html( sprintf( $priceFormat, $this->number( $costs, $precision ), $priceCurrency ) ); ?></td>
				<?php if( $modify ) : ?>
					<td class="action"></td>
				<?php endif; ?>
	...
	<div class="details">
		...
		<?php foreach( $this->summaryBasket->getProducts() as $position => $product ) : $totalQuantity += $product->getQuantity(); ?>
			...
			<div class="product <?= ( isset( $errors['product'][$position] ) ? 'error' : '' ); ?>">
				...
				<div class="mobile-quantity">  <a class="quantity-txt"> <?= $enc->html( $this->translate( 'client', 'Quantity' ), $enc::TRUST ); ?></a>
		...
		<?php if( ( $costs = $this->get( 'summaryCostsDelivery', 0 ) ) > 0 ) : ?>
			...
			<div class="delivery">
				...
				<a><?= $enc->html( $this->translate( 'client', 'Shipping' ) ); ?></a>

				<a class="price"><?= $enc->html( sprintf( $priceFormat, $this->number( $costs, $precision ), $priceCurrency ) ); ?></a>
		...
		<?php if( $priceTaxflag === true ) : ?>
			...
			<div class="total">			
				...
				<a class="price"><?= $enc->html( sprintf( $priceFormat, $this->number( $this->summaryBasket->getPrice()->getValue() + $this->summaryBasket->getPrice()->getCosts(), $precision ), $priceCurrency ) ); ?></a>
		...
		<?php if( $priceTaxflag === false ) : ?>
			...
			<div class="total">
				...
				<a class="price"><?= $enc->html( sprintf( $priceFormat, $this->number( $this->summaryBasket->getPrice()->getValue() + $this->summaryBasket->getPrice()->getCosts() + $this->summaryBasket->getPrice()->getTaxValue(), $precision ), $priceCurrency ) ); ?></a>
...
<div class="your-order-box gray-bg  " >
<h5 class="section-content__title"><?= $enc->html( $this->translate( 'client', 'Your Order' ), $enc::TRUST ); ?></h5>
                            <div class="your-order-product-info">
                                <div class="your-order-top d-flex justify-content-between">
                                    <h6 class="your-order-top-left font--bold"><?= $this->translate( 'client', 'Product' ); ?></h6>
                                    <h6 class="your-order-top-right font--bold"><?= $this->translate( 'client', 'Total' ); ?></h6>
								</div>
								...
								?>
										...
										<a href="<?= $enc->attr( $url ); ?>" class="your-order-middle-left font--semi-bold">
										<?= $enc->html( $product->getName(), $enc::TRUST ); ?> X 
								...
								<span class="your-order-middle-right font--semi-bold"><?= $enc->html( sprintf( $priceFormat, $this->number( $product->getPrice()->getValue() * $product->getQuantity(), $precision ), $priceCurrency ) ); ?></span>
									</li>
									<?php endforeach; ?>
                                
								</ul>
                                <div class="your-order-bottom d-flex justify-content-between">
									<h6 class="your-order-bottom-left font--bold"><?= $this->translate( 'client', 'Shipping' ); ?></h6>
									<?php if( ( $costs = $this->get( 'summaryCostsDelivery', 0 ) ) > 0 ) : ?>
		
										<span class="your-order-bottom-right font--semi-bold"><?= $enc->html( sprintf( $priceFormat, $this->number( $costs, $precision ), $priceCurrency ) ); ?></span>
                                ...
                                <div class="your-order-total d-flex justify-content-between">
									<h5 class="your-order-total-left font--bold"><?= $this->translate( 'client', 'Total' ); ?></h5>
									<?php if( $priceTaxflag === true ) : ?>
	
										<h5 class="your-order-total-right font--bold"><?= $enc->html( sprintf( $priceFormat, $this->number( $this->summaryBasket->getPrice()->getValue() + $this->summaryBasket->getPrice()->getCosts(), $precision ), $priceCurrency ) ); ?></h5>
					...
					<?php $totalQuantity = 0; ?>
						...
						<?php if( ( $this->summaryBasket->getProducts()->count() >0 ) ) : ?>

						<?php $product=$this->summaryBasket->getProducts()->last() ?>				
							<div class="col-md-7">
							<div class="row">
						<div class="modal__product-img col-md-4 ">
							<?php if( ( $url = $product->getMediaUrl() ) != '' ) : ?>
								<img class="img-fluid" src="<?= $enc->attr( $this->content( $url ) ); ?>" />
							<?php endif; ?>
						</div>
						...
						<div class="details col-md-8">
							...
							?>
							<h6 class="productmsg"><a class="product-name" href="<?= $enc->attr( $url ); ?>"><?= $enc->html( $product->getName(), $enc::TRUST ); ?></a>
							...
							<div class="link--green link--icon-left"><i class="fal fa-check-square"></i><?= $enc->html( $this->translate( 'client', 'Added to cart successfully!' ), $enc::TRUST ); ?></div>
			    ...
			    <?php foreach( $this->summaryBasket->getProducts() as $position => $product ) : $totalQuantity += $product->getQuantity(); ?><?php endforeach;?>
				<div class="col-md-5 modal__border">
					<div class="quantity">
						<?php if( $priceTaxflag === true ) : ?>
							<ul class="modal__product-shipping-info">
                                <li class="link--icon-left"><i class="icon-shopping-cart"></i> <?= $enc->html( $this->translate( 'client', 'There Are' ), $enc::TRUST ); ?> &nbsp; <a href="/shop/basket"> <?= $enc->html( sprintf( $this->translate( 'client', '%1$d article', '%1$d articles', $totalQuantity ), $totalQuantity ) ); ?> </a> &nbsp; <?= $enc->html( $this->translate( 'client', 'In Your Cart.' ), $enc::TRUST ); ?></li>
                                <li><?= $enc->html( $this->translate( 'client', 'TOTAL PRICE:' ), $enc::TRUST ); ?> <span><?= $enc->html( sprintf( $priceFormat, $this->number( $this->summaryBasket->getPrice()->getValue() + $this->summaryBasket->getPrice()->getCosts(), $precision ), $priceCurrency ) ); ?></span></li>
                                <li><a href="<?= url('/shop/basket')?>" class=" text-underline color-green" data-dismiss="modal"><?= $enc->html( $this->translate( 'client', 'Go To Basket' ), $enc::TRUST ); ?></a></li>
                        	</ul>
