...
<?php endif; ?>
	...
	<?php if ( $productItem->getProperties( 'package-width' )->first()){ ?>
		...
		<?php /*print_r(get_class_methods($productItem));
		
		$weight = $productItem->getProperties( 'package-protein' )->first();
		echo $weight;*/
		?>
 
		<?php if( !($productItemProperty = $productItem->getProperties( 'package-protein' ))->isEmpty() ) : ?>
			<?php /* dd( $productItem->getProperties( 'package-protein' ))*/?>
			<?php foreach($productItemProperty as $productItemPropertyValues) :
				
				/*echo $productItemPropertyValues;*/
